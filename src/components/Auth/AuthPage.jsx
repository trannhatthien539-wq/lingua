...
export default function AuthPage({ onGuest }) {
  ...
  return (
    <main className="min-h-screen bg-[#f5f7f3] p-4 text-ink dark:bg-[#121715] dark:text-white sm:p-6 lg:p-10">
      <div className="mx-auto grid min-h-[calc(100vh-2rem)] max-w-6xl overflow-hidden rounded-[2rem] border border-ink/[0.08] bg-white shadow-2xl shadow-ink/10 dark:border-white/[0.08] dark:bg-[#1b211f] lg:grid-cols-[1.05fr_0.95fr] lg:min-h-[calc(100vh-5rem)]">
        <section className="relative overflow-hidden bg-ink p-8 text-white sm:p-12 lg:p-16">
          ...
          <div className="relative flex h-full flex-col">
            ...
            <div className="my-auto max-w-lg py-16">
              <p className="text-xs font-bold uppercase tracking-[0.2em] text-lime/70">Language workspace</p>
        ...
        <section className="flex items-center p-6 sm:p-12 lg:p-16">
          <div className="mx-auto w-full max-w-md">
            ...
            <div className="my-7 flex items-center gap-3 text-[11px] font-bold uppercase tracking-widest text-ink/30 dark:text-white/30"><span className="h-px flex-1 bg-ink/10 dark:bg-white/10" />Hoặc email<span className="h-px flex-1 bg-ink/10 dark:bg-white/10" /></div>
            <form onSubmit={submitEmail} className="space-y-4">
              ...
              <button disabled={loading} className="flex w-full items-center justify-center gap-2 rounded-xl bg-ink py-3.5 text-sm font-bold text-white transition hover:opacity-90 disabled:opacity-60 dark:bg-lime dark:text-ink">{loading ? "Đang xử lý..." : mode === "signin" ? "Đăng nhập" : "Tạo tài khoản"}<ArrowRight size={16} /></button>
            </form>
            ...
            <p className="mt-8 flex items-center justify-center gap-1.5 text-[11px] text-ink/35 dark:text-white/35"><Check size={13} /> Dữ liệu được đồng bộ riêng theo tài khoản</p>
          </div>
        </section>
      </div>
    </main>
