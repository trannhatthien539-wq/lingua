...
import { LogIn, LogOut, Sparkles, UserRound, X } from 'lucide-react';

export default function MobileHeader({ user, streak, onGoogleLogin, onSignOut, notice }) {
  ...
  return <>
    <header className="sticky top-0 z-40 flex items-center justify-between border-b border-zinc-200 bg-white/90 px-4 py-2.5 backdrop-blur-md dark:border-white/10 dark:bg-[#151a18]/90 md:hidden">
      ...
      {user ? <button onClick={() => setOpen((value) => !value)} className="grid h-9 w-9 shrink-0 place-items-center overflow-hidden rounded-full bg-lime text-sm font-black text-ink" aria-label="Mở tài khoản">{user.photoURL ? <img src={user.photoURL} alt="" className="h-full w-full object-cover" /> : initials}</button> : <button onClick={onGoogleLogin} className="flex items-center gap-1.5 rounded-full border border-ink px-3 py-2 text-xs font-bold dark:border-lime dark:text-lime"><LogIn size={14} />Đăng nhập</button>}
    </header>
