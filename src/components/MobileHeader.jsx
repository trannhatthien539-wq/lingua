...
import { LogIn, LogOut, Moon, Sparkles, Sun, X } from 'lucide-react';
import SyncStatusBadge from './ui/SyncStatusBadge';

export default function MobileHeader({ user, streak, onGoogleLogin, onSignOut, theme, onToggleTheme }) {
  const [open, setOpen] = useState(false);
  const initials = (user?.displayName || user?.email || 'Guest').trim().slice(0, 1).toUpperCase();

  return <>
    <header className="sticky top-0 z-40 flex items-center justify-between border-b border-ink/10 bg-slab/95 px-4 py-2.5 backdrop-blur-md dark:border-white/10 dark:bg-dark1/95 md:hidden">
      <div className="flex min-w-0 items-center gap-2.5"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-ink text-lime dark:bg-lime dark:text-ink"><Sparkles size={16} /></span><div className="min-w-0"><p className="font-display text-sm font-bold">lingua.</p><p className="truncate text-xs text-ink/60 dark:text-white/55">{streak?.currentStreak > 0 ? `${streak.currentStreak} ngày chuỗi` : 'Bắt đầu học hôm nay'} · {new Date().toLocaleDateString('vi-VN', { day: '2-digit', month: '2-digit' })}</p></div></div>
      <div className="flex items-center gap-2"><SyncStatusBadge user={user} compact /><button onClick={onToggleTheme} className="grid h-11 w-11 place-items-center rounded-xl text-ink/60 transition hover:bg-ink/[0.06] hover:text-ink dark:text-white/60 dark:hover:bg-white/10 dark:hover:text-white" aria-label="Đổi chế độ sáng tối">{theme === 'dark' ? <Sun size={17} /> : <Moon size={17} />}</button>{user ? <button onClick={() => setOpen((value) => !value)} className="grid h-11 w-11 shrink-0 place-items-center overflow-hidden rounded-full bg-lime text-sm font-black text-ink" aria-label="Mở tài khoản">{user.photoURL ? <img src={user.photoURL} alt="" className="h-full w-full object-cover" /> : initials}</button> : <button onClick={onGoogleLogin} className="flex h-11 items-center gap-1.5 rounded-xl border border-ink/15 px-3.5 text-sm font-bold text-ink/80 dark:border-white/20 dark:text-white/80"><LogIn size={15} />Đăng nhập</button>}</div>
    </header>
