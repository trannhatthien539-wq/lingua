...
export default function MobileHeader({ user, streak, onGoogleLogin, onSignOut, theme, onToggleTheme }) {
  ...
  return <>
    <header className="no-print sticky top-0 z-40 flex items-center justify-between border-b border-ink/10 bg-slab/95 px-4 py-2.5 backdrop-blur-md dark:border-white/10 dark:bg-dark1/95 md:hidden">
      <div className="flex min-w-0 items-center gap-2.5"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-lime/25 dark:bg-lime/20"><AppMark className="h-7 w-7" /></span><div className="min-w-0"><p className="font-display text-sm font-bold">lingua.</p><p className="truncate text-xs text-ink/60 dark:text-white/55">{streak?.currentStreak > 0 ? `${streak.currentStreak} ngày chuỗi` : 'Bắt đầu học hôm nay'} · {new Date().toLocaleDateString('vi-VN', { day: '2-digit', month: '2-digit' })}</p></div></div>
