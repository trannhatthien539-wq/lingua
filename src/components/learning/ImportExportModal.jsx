  ...
  const handleFile = async (event) => {
    ...
    try {
      if (file.name.toLowerCase().endsWith(".apkg")) {
        ...
        setTitle(result.title); setPreview(result.cards); setMessage(`${result.cards.length} thẻ, ${result.imageCount} ảnh, ${result.audioCount} âm thanh.`); setImportMode("file");
      } else {
        ...
        setPreview(importedCards.filter((card) => card.word).map((card) => ({ ...card, status: card.status || "new", level: card.level || "B1" })));
