...
/** Bốc 6 thẻ cho một lượt ghép; gọi lại khi "Chơi lại" để đổi bộ thẻ. */
const pickRound = (cards) => shuffle(cards.map(sanitizeCard)).slice(0, 6);

export default function MatchingView({ deck, cards, onClose, onChangeMode, onUpdateCard, onStudyActivity }) {
  // Chọn thẻ bằng state (khởi tạo 1 lần cho mỗi lượt) thay vì useMemo phụ thuộc biến đếm
  // "round" không dùng trong thân hàm — cách cũ khiến lint cảnh báo dependency thừa.
  const [selectedCards, setSelectedCards] = useState(() => pickRound(cards));
  ...
  // Timer hiệu ứng thẻ sai: giữ trong ref để dọn khi unmount hoặc "Chơi lại",
  // tránh timeout cũ gọi setWrong/setFirst đè lên lượt chơi mới.
  ...
  const choose = (tile) => {
    ...
    if (first.pairId === tile.pairId && first.type !== tile.type) {
      ...
      wrongTimerRef.current = window.setTimeout(() => { setWrong([]); setFirst(null); }, 650);
    }
  };
  if (complete) return <StudySummary title={`Matching · ${deck.title}`} score={matched.length} total={selectedCards.length} duration={`${elapsed}s`} onReplay={() => { window.clearTimeout(wrongTimerRef.current); setSelectedCards(pickRound(cards)); setFirst(null); setMatched([]); setWrong([]); setStartedAt(Date.now()); setElapsed(0); }} onChangeMode={onChangeMode} onClose={onClose} />;
