  ...
  return (
    <div className="fixed inset-0 z-[80] overflow-y-auto bg-[#f5f7f3] p-4 text-ink dark:bg-[#121715] dark:text-white sm:p-8">
      <div className="mx-auto max-w-3xl">
        ...
        <div className="mt-10 [perspective:1200px]"><button onClick={() => setFlipped((value) => !value)} className={`relative min-h-[390px] w-full [transform-style:preserve-3d] transition-transform duration-500 ${flipped ? "[transform:rotateY(180deg)]" : ""}`} aria-label="Lật flashcard">
          <div className="panel absolute inset-0 flex flex-col items-center justify-center p-8 [backface-visibility:hidden]"><p className="eyebrow">Mặt trước</p><p className="mt-7 font-display text-5xl font-bold sm:text-7xl">{currentCard.word}</p><button onClick={(event) => { event.stopPropagation(); speak(currentCard.word, `${currentCard.id}-word`); }} className={`mt-8 flex items-center gap-2 rounded-xl bg-lime px-4 py-3 text-sm font-bold text-ink ${speaking === `${currentCard.id}-word` ? "animate-pulse" : ""}`}><Volume2 size={17} />Phát âm</button><p className="mt-8 text-xs text-ink/40 dark:text-white/40">Bấm Space hoặc click để xem nghĩa</p></div>
          <div className="panel absolute inset-0 flex [transform:rotateY(180deg)] flex-col items-center justify-center p-8 text-center [backface-visibility:hidden]"><p className="eyebrow">Mặt sau</p><p className="mt-5 text-lg text-sage">{currentCard.ipa || "Chưa có IPA"}</p><p className="mt-2 font-display text-3xl font-bold">{currentCard.meaning}</p><button onClick={(event) => { event.stopPropagation(); speak(example, `${currentCard.id}-example`); }} className={`mt-7 flex max-w-lg items-start gap-2 rounded-xl bg-ink/[0.06] p-4 text-left text-sm leading-6 dark:bg-white/10 ${speaking === `${currentCard.id}-example` ? "text-sage" : ""}`}><Volume2 size={17} className="mt-1 shrink-0" />{example}</button><p className="mt-5 text-xs text-ink/40 dark:text-white/40">Nhấn số 1, 2 hoặc 3 để đánh giá</p></div>
        </button></div>
