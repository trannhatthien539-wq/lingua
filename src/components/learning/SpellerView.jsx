...
export default function SpellerView({ deck, cards, onClose, onChangeMode, onUpdateCard, onStudyActivity }) {
  ...
  const submit = async (event) => {
    ...
    await onUpdateCard(card.id, { status: "mastered", interval: 5, nextReviewDate: addDaysKey(5), lastStudiedDate: dateKey(), repetition: Number(card.repetition || 0) + 1 });
  ...
  return <div className="fixed inset-0 z-[90] overflow-y-auto bg-mist p-5 text-ink dark:bg-dark1 dark:text-white sm:p-10"><div className="mx-auto max-w-2xl"><header className="flex items-center justify-between"><div><p className="eyebrow">Nghe &amp; viết · {deck.title}</p><p className="mt-1 text-sm font-bold">Từ {index + 1}/{safeCards.length}</p></div><button onClick={onClose} className="icon-btn" aria-label="Đóng"><X size={19} /></button></header><div className="mt-4 h-2 rounded-full bg-ink/10 dark:bg-white/10"><div className="h-full rounded-full bg-sage transition-all" style={{ width: `${(index / safeCards.length) * 100}%` }} /></div><section className="panel mt-10 p-8 text-center"><div className="mx-auto grid h-14 w-14 place-items-center rounded-full bg-lime text-ink"><Headphones size={24} /></div><p className="eyebrow mt-5">Nghe và gõ lại</p><button onClick={getPlayableAudio(card)} className="mt-5 flex mx-auto items-center gap-2 rounded-xl bg-lime px-5 py-3 text-sm font-bold text-ink"><Volume2 size={17} />Phát lại âm thanh</button><p className="mt-7 text-lg font-bold">{card.meaning}</p><p className="mt-2 text-sm text-sage">{card.ipa || "IPA chưa có"}</p><form onSubmit={submit} className="mx-auto mt-8 flex max-w-md gap-2"><input autoFocus value={value} onChange={(event) => setValue(event.target.value)} className="min-w-0 flex-1 rounded-xl border border-ink/10 bg-transparent px-4 py-3 text-sm outline-none dark:border-white/10" placeholder="Gõ từ tiếng Anh..." /><button className="rounded-xl bg-ink px-5 py-3 text-sm font-bold text-white dark:bg-lime dark:text-ink">Kiểm tra</button></form>{feedback && <p className="mt-4 flex items-center justify-center gap-2 text-sm font-bold text-red-600 dark:text-red-300"><X size={16} />{feedback}</p>}{!feedback && value && <p className="mt-4 text-xs text-ink/40 dark:text-white/40"><Check size={13} className="mr-1 inline" />Nhấn Enter để kiểm tra</p>}</section></div></div>;
}
